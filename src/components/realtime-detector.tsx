...
const WebcamStream: React.FC<{ initiallyActive?: boolean; videoPath?: string }> = ({
  ...
  const handleLearnObject = (object: string) => {
    if (!learnedObjects.includes(object)) {
      setLearnedObjects((prev) => [...prev, object]);
      toast({ title: 'Learned New Object', description: `Learned object: ${object}`, variant: 'info' });
    }
  };

  const switchToLearningMode = () => setMode('Learning');
  const switchToGameMode = () => {
    if (learnedObjects.length === 0) {
      toast({ title: 'Error', description: 'No objects learned yet. Please run learning mode first.', variant: 'warning' });
      return;
    }
    setMode('Game');
    chooseNextObject();
  ...
  const chooseNextObject = () => {
    const availableObjects = learnedObjects.filter((obj) => obj !== currentObject);
    if (availableObjects.length > 0) {
      setCurrentObject(availableObjects[Math.floor(Math.random() * availableObjects.length)]);
    } else {
      toast({ title: 'No Objects', description: 'No available objects to choose from.', variant: 'info' });
    }
  };
